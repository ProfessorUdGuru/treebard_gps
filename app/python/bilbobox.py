...
# This is like a combobox but only works with the mouse. Tab key, arrow keys, etc. are not used. It also opens the dropdown list on hover instead of click. I like it but I think it will annoy people by not doing what they expect so I'm putting it back in mothballs for now.
...
# post CanvasScrolled as answer and on forum.

# older notes for forum and/or docstrings
...
# 1) it's ttk so you have to stand on your head to style it. The ttk widgets were designed to look more "modern" and to enforce Windows themes, so there are many options not available. So ttk.Style often does nothing and the whole idea of trying to configure ttk widgets quickly becomes a waste of time--especially when configuring tk widgets by class is easy. When they added a Combobox and a Notebook to Tkinter, they should have added them to Tkinter and made a ttk version for people who like to work twice as hard to style widgets to look like what was modern when ttk was born.
# 2) you can't bind to an event in the dropdown so you have to use Return key (not spacebar) to select a highlighted widget because that's all the creators of ttk gave us
# 3) it doesn't drop down unless you click that teeny arrow.
# 4) on focus in the only way you know you're focused into the combo is if you notice the background color changed on that teeny arrow
# 5) if the combobox is close to the bottom of the screen, the dropdown will fly up instead of down, which is correct, but the down button has to be used to make it fly up which is anti-intuitive. The up OR down button should always open the dropdown whether it needs to fly up or fly down.
# 6) When using arrows to go up and down the dropdown list, when it gets to the frame or bottom the traversal stops instead of going around to the other end.
...
# By the way, the ttk.Combobox dropdown is a tk.Listbox. The Listbox is one of the few tk widgets I've neglected to use because using it is weird, inconsistent with using the other tk widgets, complicated. It doesn't always play well with the Text widget, for example.