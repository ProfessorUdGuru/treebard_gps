...
class NotesDialog(Toplevel):
    ...
    def add_links(self):
        ...
        self.all_names = EntryAutoHilited.create_lists(people)
...
# DOCS: (places: links) Where Treebard places meets Treebard notes, a special kind of complexity is formed which has to be dealt with appropriately so the user sees a simple interface while the data details are handled carefully behind the scenes. Any given note can be linked to any number of elements, including places. Places can have more than one containing (parent) place. But the note-to-place link isn't to a nesting like "Aspen, Pitkin County, Colorado, USA", for which there is no ID, but to a nested pair like "Aspen, Pitkin County" for which there is an ID. This is a link to a single place: a place called Aspen whose immediate parent happens to be a place called Pitkin County, but the actual link is to the place called Aspen, whose parent is just part of that single place's description. If Aspen was in a different county at an earlier point in its history, then a different ID is available for "Aspen, Other County".
