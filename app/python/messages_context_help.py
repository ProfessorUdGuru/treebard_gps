...
# rcm_widgets = (
    ...
    # self.order)
notes_dlg_help_msg = (
    ('Type the name of a topic i.e. title for this note. For example, if the note is about a wedding conclusion, there could be notes with topics such as "Uncle Buck--Incident at Reception", "Change of Venue at Last Minute" and "Ringbearer Got Lost"', 
    'Notes Dialog: Note Topic'),
    ("Type or paste text of any length. The Particulars column in the conclusions table can hold one short note, whereas this notes field can be used for any amount of detail. On the conclusions table, in the row pertaining to a given conclusion, the button in the Notes column will read '...' if any notes exist for that conclusion. Clicking a blank button will open the Notes Dialog so the first note can be created. There's no SUBMIT button in the dialog since all changes made are saved automatically as soon as they're made.", 
    "Notes Dialog: Note Input"),
    ("All notes need a topic which is unique throughout the current tree. This is part of the feature which allows any note to be linked to more than one entity so that copy/pasting and duplicate storing of notes is unnecessary. The left panel is a table of contents listing note topics linked to this conclusion or other element. Clicking a topic in the left panel of the notes dialog opens that note for reading or editing.",
    "Notes Dialog: Table of Contents"),
    ("Each note can be linked to any number of elements. If you open the Note Dialog by clicking a button in the Notes row of the conclusions table, the notes you create in that dialog will be automatically linked to the conclusion corresponding to the row that the Note button was in. The LINK TO... button in the Notes Dialog opens a dialog wherein you can link the current note to any other element in the tree, such as a person, place, citation, image, to-do item, chart, report, etc.",
...
# rcm_widgets = (
    ...
    # self.top_pic_button, self.findings_table.conclusion_input, 
    # self.att.finding_input, self.fontpicker.output_sample, 
...
main_help_msg = (
    ("Use this field to change the current person (the person whose information is displayed in the Person Tab) to a person who exists in the tree or to a new person who you want to add to the tree. When you start typing, Treebard will try to guess which person you want based on what you type as well as the name used last time you typed into a person autofill input. If Treebard guesses right, click the OK button. If you type a name that Treebard doesn't recognize, the Add Person dialog will open. If you're trying to enter a name that already exists in the tree, instead of changing the current person to the John Smith that already exists, enter '+John Smith' or 'John Smith+' and the Add Person dialog will open. You can also enter a person's unique Treebard ID number if you know it, like this: '#670'. If you don't know the ID number, you can press the button that says 'ADD/FIND'. The Person Search dialog will open, and you can use it to search for the person you tried to enter, for example if you forgot how their name was spelled.\n\nHere you will see the same person filled in, and the search results table below will be empty. Delete and retype one character in the name, and all the John Smiths will appear in the table, each with their own row. In the row will also appear any birth or death dates, and names of father and mother, if you've entered this information already. In this way you'll be able to tell which John Smith you were after. Just click his name and he will become the new current person.\n\nIf you're still not sure, you can point the mouse at the names or ID numbers, and more information will appear in a tooltip in the form of any alternate names, nicknames, etc. that you have entered for each John Smith. This system allows you to enter several people by the same name without having to compromise the facts by entering them as 'John Smith son of George' or anything like that.\n\n",
    ...
    ("The conclusions table on the Person Tab and the Attributes Table on the Attributes Tab are identically designed, but contain different information. If you enter an attribute, it will end up on the Attributes Table, even if you create it from the Person Tab, and vice versa. More information on how to tell the difference is given below. The new conclusions input fills in automatically with values from a list of event types that includes both events and attributes and can be used equally to describe assertions and conclusions.",
        "Person Tab: New Conclusion Input"),
    ("The conclusions table on the Person Tab is the main display area for what other genealogy apps might call event, attributes, or facts. If you leave the date blank, it will end up at the bottom part of the table until you add a date. More information on how to tell the difference is given below. The new conclusions input fills in automatically with values from a list that includes both events and attributes, since the line between these two elements is often fuzzy.\n\nWhen you start typing a new conclusion as an event type in the new conclusion input, it will autofill with event types that match what you're typing. When the correct event type has filled into the input, press the NEW CONCLUSION button to display the new conclusion in the table. If you fill in a date, the conclusion will move up with the other dated conclusions, sorted there by date. See the final paragraph in this topic for more information on how to get the conclusion to display in the right part of the table. See the context help topics in the date preferences tab for a lot more information.\n\nIf the conclusion is about a couple event such as wedding, elopement, separation, etc., then an input will appear for a partner's name in the families table, which is above the conclusions table. Here you will be able to input the name of the other person such as the spouse, if you know it.\n\nIn Treebard when a man or woman becomes a parent biologically, that person experiences an 'offspring' event. Treebard automatically creates an offspring event row in the parents' conclusions tables as well as the child's birth event in the child's conclusions table. If you try to create an offspring event in the new conclusion input, Treebard will notify you that this can only be done automatically by Treebard.\n\nTreebard is aware that separating events and attributes by whether or not the event is dated is a fuzzy delineation, but you have a lot of control over where a conclusion actually appears. As an example, my grandparents all had gray hair when they were old, but my paternal grandmother also had gray hair when she was young. It would be interesting to note as an event that her hair turned gray 'about 1920 to 1930'. But for most people whose normal hair color was known, it would be silly to add a date to say when someone had a hair color; that would be a natural attribute. But take job vs. career for example. Either one could be on the conclusions table by giving it a date, even if an estimated or approximate date. But to say someone was a farmer by trade could go either way. If the statement is from a dated source, personally I would prefer to see it as a dated event, even though it is more like an attribute. But if you wanted, you could just add a date informally to the particulars column in the conclusions table, in which case the conclusion would be found at the bottom of the table, filed alphabetically by event type. In spite of any shortcomings, Treebard feels that this is a better system than interfiling dated and undated events on a single table which then tries to sort itself by date and can't.",
        "Person Tab: New Conclusion Input"),
    ...
    ("This topic will introduce the conclusions table and Attributes Table, which are located on the Persons Tab and Attributes Tab respectively, and are in most ways identically structured. For more information about the columns, consult the context help topics at the various column heads on either table. Both tables are in reference to the Current Person.\n\nThe conclusions table is sorted by date, however even if dates are obviously wrong, birth will always be first, death will always be last except for burial and other after-death events which will be in order of date below the row for death. Everything else will appear strictly in order. In the case of range or span dates, the first date is the one that will be sorted on.\n\nThe plain text fields in the first five columns take in put directly, but respond differently, depending on that columns validation requirements. The Age and Particulars columns are not validated, you can put any text in these fields. The Places column is an autofill input which remembers the last place you input that starts with the characters you're typing and tries to fill in what you are probably trying to type. Date columns are easy to use, validated to disallow bad dates, and formatted according to your settings in the Date Preferences Tab.\n\nThe conclusion column accepts event types including those which are new to Treebard. Couple events can be changed to different couple events and generic (non-couple) events can be changed to other generic events. For example, a marriage can be changed to a wedding or a career can be changed to an occupation. A divorce can't be changed to a death, but a residence can. In general, to change the text in a table cell, just change it, and your changes will be saved instantly. There is a SAVE menu item, as well as the usual CTRL+S that you expect, but all this does is to redraw the current person tab, since changes are saved automatically as soon as they're made, except in dialogs where there is an OK or SUBMIT button.\n\nThe last three columns contain buttons instead of plain text. The Roles and Notes columns have either plain buttons or buttons with three dots ('...') to indicate that roles and notes are linked to that conclusion. To create roles and notes when none exist, just open the dialog by clicking the blank button in the row corresponding to the relevant conclusion. The sources column says how many citations--and thus how many assertions--are linked to the conclusion which the row corresponds to.",
        "Conclusions Table: Event Type Column"),
    ("Event types are of two basic kinds: events and attributes. For example: Grandma Robertson had prematurely gray hair, so I could record hair color as a dated event with a range date such as 'between 1920 and 1930' and with 'prematurely gray' in the Particulars column. But assuming that Grandpa had brown hair most of his life, this would naturally belong in the attributes portion of the conclusions table without a date, and I don't think it matters how many of his hairs turned gray in which year.\n\nInstead of entering new rows directly into the table, you enter an event type into the new conclusion input, and Treebard creates a new undated row in the bottom part of the table.\n\nWhile Treebard's one and only criteria for deciding whether a conclusion refers to an event or an attribute is whether or not it has an official date, we recognize that reality is not this simple. But Treebard needs a date-sorted table in the persons tab in order to tell a chronological story of a person's experiences, so undated conclusions are not interfiled with dated conclusions. It's up to you, but Treebard suggests adding estimated or approximate dates to events where the exact dates are unknown, in order to get them interfiled reasonably well by date on the conclusions table. On the other hand, if you have a date for an attribute but really want that attribute with attributes and not events, just put the date in the particulars column or in a note.",
    ...
    ("Roles are a key feature in Treebard which is mostly or completely missing from most genieware applications. How many times have you felt compelled to research the odd neighbor or boarder, with the sneaking suspicion that there is a story to be found that profoundly affected your family story. The differently-surnamed adjunct person could even turn out to be a relative. Treebard believes that we can't be serious about telling a story if we're not interested in the people who our ancestors were interested in, so Treebard provides a robust system for keeping track of these auxiliary folks. They are treated the same as any other person and are easily accessible in the Roles Dialog, searchable in the Person Search, etc. For more information, consult the context help topic for the Roles Dialog.",
        "Conclusions Table: Roles Column"),
    ("Treebard lets you link the same note to any number of sources so that you don't have to copy, paste and store the same note more than once. Because of this extra attention to detail, we need a way to track notes that is meaningful to the genealogist (instead of ID numbers for example), so we have topic heading for each note. Each topic heading is unique throughout the whole tree, and the topic headings appear in a Table of Contents for the conclusion that the note is linked to. For more information, see the context help topics for the Notes Dialog.",
        "Conclusions Table: Notes Column"),
    ("The sources button displays a number indicating how many assertions are linked to that conclusion. An assertion is a claim made by a source. The Treebard won't allow two birth or death events for the same person since this is physically impossible. But sources don't always agree, so Treebard provides another layer of data that mediates between the evidence and the conclusion, and that is the assertion. If you click the SOURCES button, the assertions dialog will open. There you can view, create, and edit sources, citations, rpositories, and assertions. Since it is unrealistic to record two conclusions about the same event in the conclusions table, Treebard provides assertions where each source could conceivably suggest its own separate conclusion. So let's say you have a residence event but different sources tell a different story about where the person lived at a particular time. You can record anything you want on the conclusions table as a conclusion (except for multiple births or deaths for the same person). Then each source for that conclusion can be linked to a separate assertion. The SOURCES button in the conclusions table will count the assertions linked to that event.",
        "Conclusions Table: Sources Column"),
    ...
    'like that, notes or the Particulars column of the conclusions table are a good place to store them. In '
    ...
    'mistake in editing a date on the conclusions table makes the whole row '
    'disappear, it\'s because the bad date was deleted. This caused the '
    'row to move to the bottom part of the table with other undated rows. It can '
    'be resurrected as an event by finding it and giving it a date.', 
    ...
    'one, for example to get a conclusion to appear in chronological order in '
    'the conclusions table. (In Treebard, an undated event is treated as an attribute and will appear at the bottom of the conclusions table with other undated conclusions.) Suppose a source states that someone '
    ...
    'causing the marriage conclusion to fall reasonably close to where '
    'it belongs within the conclusions table.', 
...
# new_event_dlg_help_msg = (
    # ("Like the date inputs on the conclusions table, you can input date parts in almost any order, use a variety of abbreviations for date qualifiers, etc. Unlike the conclusions table cells, the date won't be formatted according to your preferences until the dialog closes. Then the date will be correctly formatted as it appears in the new row on the table. For more information on how to input dates, see the Dates Preferences Tab, where there are also some test input widgets where you can instantly see results of various date inputs without affecting your tree.\n\n",
        # "New Event Dialog: Date Input"),
    # ("Like the place inputs on the conclusions table, this place input will fill in automatically with whatever places are spelled like what you're entering, beginning with the most recent places you've entered in the conclusions table. When you tab out of the place input, if you've entered any place names that are new or duplicated, the New & Duplicate Places Dialog will open. See the context help topic at the Places column in the conclusions table for more information.",
        # "New Event Dialog: Place Input"),
    # ("The Particulars Input in the New Event Dialog works the same as the Particulars column in the conclusions table. This field is for very short notes or additional details. For example, in an 'educational level' attribute, you could type 'through grade 4' in the Particulars input. For larger notes, use the Notes Dialog which will be available in the new event or attribute row after you close the New Event Dialog.",
