...
# Lucien Patrick works but Erlinda doesn't. THIS IS WHERE i LEFT OFF
# create from scratch a new procedure (singular) to create offspring and alt_parentage conclusions; focus on using the main dict and not creating a bunch of stray lists; think about the notion of auto-creating births and how that makes it so much easier, then find a way to make one single symmetrical method that deals with birth and alt_birth findings the same way and keep it simple
