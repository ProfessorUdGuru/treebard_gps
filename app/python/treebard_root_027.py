...
# try deleting the whole event by deleting the partner and checking all the checkboxes, make sure persons_persons row is also being deleted, make sure the function in events_table.py is being used.
# `adding & editing partners not done here` printing when trying unsuccessfully to add James to Selina's blank partner field after deleting him only from the event
# # when deleting only james from a selina event, the partner input is blank in selina's table (correct), but when deleting only selina from a james event, there's no longer a partner input at all in james' table. The reason this happens is firstly because Selina has no other events so it was straightforward to display her relationships, and better explained by this resulting dict data:
# # None: {'sorter': [1906, 0, 0], 'partner_name': 'name unknown', 'parent_type': "Children's Mother", 'partner_kin_type': 'Partner', 'inwidg': None, 'children': [{'fpid': 95, 'birth_id': 872, 'order': '2-1', 'gender': 'unknown', 'birth': '1925', 'sorter': [1925, 0, 0], 'death': '', 'name': 'Ulrike Macie Deann Theriault', 'id': 5587}, {'fpid': 96, 'birth_id': 671, 'order': '2-1', 'gender': 'male', 'birth': 'Sep 30, 1929', 'sorter': [1929, 9, 30], 'death': '', 'name': "Joe-John O'Keefe", 'id': 5732}], 'marital_events': [{'fpid': 264, 'finding': 1154}]}, 
# # What happened above is that when the partner id became None, the missing partner data (finding id, fpid, date) was merged into the existing None partner who had 2 children, since there can only be one None as key for the same dict. This can possibly be fixed by storing partners from marital events and partners from offspring events separately in the dict. It could also possibly be tolerated, for example the workaround would be to add selina to the blank children's mother field but then she'd end up with the children. What does happen if you do this is that the blank partner field appears. Then if you try to delete selina as mother, the unlink-which-dlg opens with no checkbuttons, but clicking cancel seems to do the unlinking anyway since the Children's mother field goes back to blank and the blank partner field disappears again. You can go back and forth and it does everything predictably which is the good news.
# # The bad news is that if you auto-create a key such as 'none-1', 'none-2' etc each time a None key comes up, each unclaimed child and each unclaimed marital event will have a separate line. This might be unavoidable and it will encourage the user to group children together in logical units under unnamed parents, but this isn't necessarily a good thing. Maybe the best thing to do is to inform the user that ALL marital events and children which aren't given a named partner (even if the name is "???" or "_____") will be found in the SAME family. Which is not that bad in my opinion. The nice thing is that it's kinda accurate. GUI-wise it's confusing but I think I'll keep it this way long enough to use it and see if it leads to any serious trouble. Complicating everything with a separate partner for each unclaimed child and marital event would be truly ugly and maybe harder to undo than having them all in one family claimed by a None partner.
# # It just occurred to me that there's a correct way to deal with this situation, which is to make each person in the lumped-together-family-of-none the current person, give them the parent they shd have even if "_____", and the problem will disappear. By doing that for the two unclaimed children, I was able to make the blank partner field reappear. This is a reasonable method because all people have parents, it's reasonable to assume it and give them a storable name such as "_____".
# # But it's still wrong that something happens when a dlg with missing checkbuttons does something when cancel is pressed. this has to be fixed. However the thing it did is reasonable, it did what I would have wanted it to do. "if you try to delete selina as mother, the unlink-which-dlg opens with no checkbuttons, but clicking cancel seems to do the unlinking anyway"
# # Note: the confusion exists in the GUI, not the database. The database is perfectly clear. Trying to display all of a person's many partners and children in a single table leads to some odd situations, GUI-wise. In the case of an ordinary nuclear family with two known parents, there is no ambiguity at all, and the same goes for any situation, like a number of partners, where both partners are known or at least named if they're children, because this separates partners who have only marital events from partners who have children. I'd venture to say that giving all unknown persons the name "_____" would solve the problem completely, so I'm not going to "fix" this by making the code more complicated than it already is. The good news is that if the user WANTS to pool all his unknowns into a family unit with None as the missing partner, he can do so. ADD THIS TO A FORUM POST.
