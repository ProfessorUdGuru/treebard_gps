...
class Listbox(FrameHilited4):
    ...
    def curselection(self):
        ...
        for child in items:
            print("line", looky(seeline()).lineno, "child.cget('bg'):", child.cget('bg'))
            if child.cget('bg') == highlighted:
                selected = items.index(child)
                break
        print("line", looky(seeline()).lineno, "selected:", selected)
        return selected

    def highlight_first_last(self, evt):
        if evt.keysym not in ('Down', 'Up'):
            print("line", looky(seeline()).lineno, "evt.keysym:", evt.keysym)
            return
