...
def process_user_input(date_in, widg):
    ...
    print('306 is filtered', filtered)
    if 'and' in filtered:
        if 'to' in filtered:
            DateError(ERRORS[3])
            return
    ...
    if len(date1) > 5 or len(date2) > 5:
        DateError(ERRORS[0], widg)
        return
    print('345 date_dict is', date_dict)
...
def process_date_parts(date_dict, date1, date2, widg, unknown=None): 
    ...
    for date in (date1, date2):
        ...
        n += 1 
    print('482 date_dict is', date_dict)
...
def format_year_month_day(date_dict, idx):
    ...
    for month in (date1['month'], date2['month']):
        print('491 month is', month, 'n is', n)
        if len(month) == 0:
            n += 1
            continue
...
def apply_date_format(date_dict, widg, unknown=None):
    print('570 date_dict is', date_dict)
    print('571 unknown is', unknown)
    date_string = ''
    link = None
    date1 = ''
    date2 = ''    
            ...
            if len(date_dict['link']) != 0:
                if unknown == 2:
                    ...
                    if int(date2['year']) > 999:
                        date2['suffix'] = ''
    print('561 date_dict is', date_dict)
    format_prefixes(date_dict)
    print('563 date_dict is', date_dict)
    format_suffixes(date_dict)
    print('565 date_dict is', date_dict)
...
def make_date_string(date_list, unknown=None):
    ...
    c = b.replace('  ', ' ')
    date_string = c.strip()
    print('787 date_string is', date_string)
...
def get_date_from_db(widg):
    ...
    cur.close()
    conn.close()
    print('847 stored_date is', stored_date)
