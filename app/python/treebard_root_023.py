...
# currently changing mother works but changing father changes mother to the new person. Redo the logic so instead of running 2 queries bec of personid1 & personid2, use a single select * query where finding_id = ? and then manipulate the possibilities in python
...
# can unlink a parent by deleting and focusout of empty entry, but on trying to add new parent later to empty entry the way of getting fpid no longer works so maybe this is a big problem, isn't there a way to store this info when it's available so I don't have to run 2 queries to find one piece of data that I shd already have? SEE `            birth_fpid = cur.fetchone()[0]` line 628
# (stopped happening?) if delete a char & retype it, the id displays after the name, but then on focusout the final != orig so code runs which shd not run so error; ?split & delete if # in string? before running code so final will still equal orig? Or is this pointing out that if autofill puts something in there with a #, all the queries to find the person are not necessary? or something
