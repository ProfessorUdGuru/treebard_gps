...
# The procedure for unlinking a parent from their children--which then creates a new partner row for the unlinked parent IF there are marital events--shd be modeled for then proceeding to unlink the partner from the current person by again deleting the name. After that, deleting a child shd unlink the child from the current person's partner, while creating an unknown partner automatically for the child's new/temp parent. If the child is to be unlinked from the current person also, just delete the child again, and the temp parent will disappear.
# update_child on delete, works but should it be possible to unlink child from only the current person or from only the partner? Shouldn't there be a dialog to this effect? Look at models. I think it shd unlink from partner only, and autocreate a new partner (unknown name) and keep the child listed in the nukes table as a child of the current person. JUST A SIMPLE DIALOG WITH 4 RADIOS: UNLINK FROM PARTNER ONLY? OR UNLINK FROM CURRENT PERSON ONLY? OR UNLINK FROM BOTH PARENT?
