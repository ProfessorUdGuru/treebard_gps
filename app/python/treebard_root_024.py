...
# retest making partners incl add person which doesn't work. the new person is made but not inserted to the widget, apparently not added to the dict, and the new name doesn't become available to person widgets till app is restarted. But changing a partner who has kids or adding a partner who has kids when partner was unknown (see James Woodland and Selina Savoy) works fine, so maybe I already did the mother of children thing but not the other kind of partner, see the if/else which maybe just isn't handling the case yet.
# the delete person dialog opens unwanted if for ex you are trying to delete a name to replace it with another name. But if you highlight and replace, the delete dialog doesn't open which is nicer but it bypasses the checks. Is the right thing being done w/ the highlight & replace procedure? Also would it be better to use CTRL+Delete or something so the dialog doesn't open by accident?
